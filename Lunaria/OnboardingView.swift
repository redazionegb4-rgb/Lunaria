...
struct OnboardingView: View {
    ...
    private var welcome: some View {
        VStack(spacing: 26) {
            Spacer()
            ZStack { Circle().fill(.ultraThinMaterial).frame(width: 210, height: 210).overlay(Circle().stroke(.white.opacity(0.8))); Image(systemName: "moon.stars.fill").font(.system(size: 86)).foregroundStyle(LinearGradient(colors: [Color.lunaBlush, Color.lunaBerry, Color.lunaLilac], startPoint: .topLeading, endPoint: .bottomTrailing)) }
