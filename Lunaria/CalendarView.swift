                    ...
                    VStack(spacing: 20) {
                        ...
                        FrostCard(radius: 30, padding: 16) {
                            DatePicker("Seleziona una data", selection: $selectedDate, displayedComponents: .date)
                                .datePickerStyle(.graphical).tint(Color.lunaBerry).labelsHidden()
                        }
                        daySummary
    ...
    private var daySummary: some View {
        ...
        return FrostCard(radius: 28, padding: 20) {
            VStack(alignment: .leading, spacing: 15) {
                HStack { VStack(alignment: .leading) { Text(store.italianDate(selectedDate, style: "EEEE")).font(.caption.bold()).foregroundStyle(Color.lunaBerry); Text(store.italianDate(selectedDate, style: "d MMMM yyyy")).font(.title3.bold()) }; Spacer(); Image(systemName: "calendar.circle.fill").font(.largeTitle).foregroundStyle(Color.lunaLilac) }
                HStack { LunaBadge(title: log.flow.rawValue, icon: log.flow.icon); LunaBadge(title: "\(log.symptoms.count) sintomi", icon: "heart.text.square", tint: Color.lunaLilac) }
